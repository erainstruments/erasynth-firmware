...
{  
  ...
  {    
	...
	while (1) 
	{
        ...
        sample = myMap((float)ADC->ADC_CDR[0], 0, 4095, lower_bound, upper_bound); // Voltage is calculated.        
        sample = 10 + 20 * log10(sample); // Convert voltages to dB.
        sample = (max_log_upper_bound - sample) * 46.5454; // Slope of RFSA2113 is 46.5454.
        setDAC(valueOfDAC + (int)sample, DAC3_LE);
        if (nextFreq){ sweepERASynth(); }
  ...
  {  
	...
	while (1) 
	{
        ...
        sample = myMap((float)ADC->ADC_CDR[1], 0, 4095, lower_bound, upper_bound); // Voltage is calculated
        sample = 10 + 20 * log10(sample); // Convert voltages to dB. 
        sample = (max_log_upper_bound - sample) * 46.5454; // Slope of RFSA2113 is 46.5454      
        setDAC(valueOfDAC + (int)sample, DAC3_LE);
